...
impl InputMethodEngine {
    ...
    pub(super) fn chunked_auto_suggest(&mut self) -> Option<String> {
        ...
        let common_prefix = common_prefix_len(&old_text, &text);
        let common_suffix = common_suffix_len(&old_text, &text, common_prefix);
        let plan = ChunkPlan::compute(&old_lens, &old_text, &text, chunk_len);

        // Forward typing inside a non-full Japanese chunk is one logical
        // live-tail request, not a lexical boundary. Keep the completed
        // surface visible while the replacement request is pending, and show
        // only the newly appended reading as raw text. The worker still gets
        // the reopened full chunk below, so the eventual completion replaces
        // this projection atomically.
        let pending_forward_projection = if common_prefix == old_text.len()
            && common_suffix == 0
            && plan.lead_count < old.len()
            && let Some(reopened) = old.get(plan.lead_count)
        {
            let mut projected = old
                .iter()
                .take(plan.lead_count)
                .map(|chunk| chunk.converted.as_str())
                .collect::<String>();
            projected.push_str(&reopened.converted);
            projected.extend(text[common_prefix..].iter());
            (projected != full_reading).then_some(projected)
        } else {
            None
        };
        ...
        pending_forward_projection.or_else(|| (combined != full_reading).then_some(combined))
    }
