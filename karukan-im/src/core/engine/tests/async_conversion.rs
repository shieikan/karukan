...
fn ready_async_completion_does_not_consume_a_pass_through_key() {
    ...
    let result = engine.process_key(&press_key(Keysym::F1));
    ...
    assert!(result.actions.iter().any(|action| {
        matches!(action, EngineAction::UpdatePreedit(preedit) if preedit.text() == "converted:アイ")
    }));
}
...
fn assert_delayed_composing_auto_suggest_is_stale(
    ...
    // Drive the real composing path.  The second key creates the delayed
    ...
    let expected_mode = engine.input_mode;
    ...
    assert_eq!(engine.input_mode, expected_mode);
...
#[test]
fn sentence_explicit_completion_filters_model_script_variants() {
    ...
    assert_eq!(candidate_texts(&engine), vec!["今日はいい天気"]);
}
...
fn explicit_completion_preserves_selected_candidate_identity() {
    let mut engine =
        explicit_completion_engine(vec!["model-first".to_string(), "model-second".to_string()]);
    engine.state.candidates_mut().unwrap().select(1);
    engine.submit_async_conversion(
        crate::core::engine::async_conversion::AsyncRequestKind::Explicit,
        false,
    );
...
fn delayed_model_preserves_counter_selection_and_page_identity() {
    ...
    engine.state.candidates_mut().unwrap().select(8);
    ...
    assert_eq!(
        before.selected().unwrap().source_label.as_deref(),
        Some("🔄 変換")
    );
    ...
    assert_eq!(
        after.selected().unwrap().source_label.as_deref(),
        Some("🔄 変換")
    );
