...
fn load_learning_cache(enabled: bool, config: LearningConfig) -> Option<LearningCache> {
    if !enabled {
        return None;
    }
    ...
    let Some(path) = Settings::learning_file() else {
        ...
        return Some(LearningCache::new(config));
    };

    if path.exists() {
        match LearningCache::load(&path, config) {
            Ok(cache) => {
                debug!(
